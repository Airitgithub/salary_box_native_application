    ...
    const handleNext = () => {
        ...
        if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{6,}$/.test(password)) {
            Toast.show({
                type: 'error',
                text1: 'Weak Password',
                text2: 'Password must be at least 6 characters long & include uppercase, lowercase, and a number',
                position: 'top',
                visibilityTime: 3000,
                topOffset: 50,
                style: { backgroundColor: COLORS.red },
        ...
        setTimeout(() => {
            ...
            navigation.navigate("LoginPinScreen", { phoneNumber: phoneNumber }); // Pass phoneNumber as a param
        }, 2000);
    };
